...
@app.route('/' + TOKEN, methods=['POST'])   ### функция из оф.документации Heroku
def get_message():
    bot.process_new_updates([telebot.types.Update.de_json(request.stream.read().decode('utf-8'))])
    return 'Python Telegram Bot', 200


@app.route('/')   ### функция из оф.документации Heroku
def main():
    bot.remove_webhook()
    bot.set_webhook(url='https://test-bot-python-june.herokuapp.com/' + TOKEN)
    return 'Python Telegram Bot', 200
